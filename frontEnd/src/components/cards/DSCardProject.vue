<template>
  <router-link :to="`/projects/${data.id}`">
    <div class="w-full max-w-sm overflow-hidden bg-white rounded-2xl shadow-md p-3 cursor-pointer">
      ...
      <div class="px-6 py-4">
        <p class="pt-2 text-2xl text-primary font-semibold">{{ data.title }}</p>
        ...
        <div class="flex items-center justify-between">
          ...
          <span class="text-text -ml-8"> Hace 5 dias </span>
          <svg
            class="w-4 h-4"
            fill="none"
            stroke="currentColor"
          ...
          <span class="text-text -ml-8"> 10 Miembros </span>
        </div>
        <p class="py-2 text-text font-light">
          {{ data.description }}
        </p>
      </div>

      <div
        class="flex flex-row px-6 pb-4 gap-3 flex-wrap w-full md:w-auto md:text-left text-center"
      >
        <span
          v-for="i in data?.stacks"
          :key="i"
